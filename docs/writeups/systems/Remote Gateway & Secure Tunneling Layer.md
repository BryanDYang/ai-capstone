...
- **Pipeline Feedback:** Streams live stage transitions (_"Transcribing..."_, _"Diarizing..."_, _"Extracting Tasks..."_) to the UI while the sequential worker processes audio on Apple Silicon.
...
| ------------ | ---------- | ----------------- | ------------------------ | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/health`    | `GET`      | No                | None                     | `200 OK`<br><br>`502 Bad Gateway`<br><br>`503 Service Unavailable` | Lightweight heartbeat probe to verify that the tunnel daemon is active and FastAPI is accepting connections before dispatching heavy pipeline workloads. |
